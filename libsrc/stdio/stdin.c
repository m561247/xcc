...
typedef struct OUTFILE {
  FILE file;
  unsigned char wwork[72];
} OUTFILE;

static INFILE  _stdin  = {.file={.iof = IOF, .flush = _fflush, .fd = STDIN_FILENO,  .flag = FF_READ, .rbuf = _stdin.rwork, .rcapa = sizeof(_stdin.rwork)}};
static OUTFILE _stdout = {.file={.iof = IOF, .flush = _fflush, .fd = STDOUT_FILENO, .flag = FF_WRITE, .wbuf = _stdout.wwork, .ws = sizeof(_stdout.wwork)}};
static OUTFILE _stderr = {.file={.iof = IOF, .flush = _fflush, .fd = STDERR_FILENO, .flag = FF_WRITE, .wbuf = _stderr.wwork, .ws = sizeof(_stderr.wwork)}};
